...
class Modem:
    ...
    def __init__(self, constellation, reorder_as_gray=True):
        ...
        if reorder_as_gray:
            ...
            gray_code_sequence_array = fromiter(
                (int(g, 2) for g in gray_code_sequence), int, len(constellation)
            )
    ...
    def modulate(self, input_bits):
        ...
        mapfunc = np.vectorize(
            lambda i: self._constellation[
                bitarray2dec(input_bits[i : i + self.num_bits_symbol])
            ]
        )
    ...
    def demodulate_v2(self, input_symbols, demod_type="hard", noise_var=1.0):
        """
        Fast demodulation function
        """
        if demod_type == "hard":
            # Vectorized hard decision - same as original but more explicit
            distances = np.abs(input_symbols[:, None] - self._constellation[None, :])
            index_list = np.argmin(distances, axis=1)
            demod_bits = dec2bitarray(index_list, self.num_bits_symbol)

        elif demod_type == "soft":
            # Fully vectorized soft decision
            num_symbols = len(input_symbols)
            demod_bits = np.zeros(num_symbols * self.num_bits_symbol)

            # Compute all distances at once: (num_symbols, num_constellation_points)
            distances_sq = (
                np.abs(input_symbols[:, None] - self._constellation[None, :]) ** 2
            )

            # Compute exponentials once
            exp_terms = np.exp(
                -distances_sq / noise_var
            )  # (num_symbols, num_constellation_points)

            # Pre-compute bit patterns for all constellation points
            constellation_indices = np.arange(len(self._constellation))
            bit_patterns = np.zeros(
                (len(self._constellation), self.num_bits_symbol), dtype=bool
            )
            for i in range(self.num_bits_symbol):
                bit_patterns[:, i] = (constellation_indices >> i) & 1

            # Vectorized LLR computation
            for bit_pos in range(self.num_bits_symbol):
                # Masks for bit=1 and bit=0 at this position
                bit_1_mask = bit_patterns[:, bit_pos]  # (num_constellation_points,)
                bit_0_mask = ~bit_1_mask

                # Sum exponentials for bit=1 and bit=0
                llr_num = np.sum(exp_terms[:, bit_1_mask], axis=1)  # (num_symbols,)
                llr_den = np.sum(exp_terms[:, bit_0_mask], axis=1)  # (num_symbols,)

                # Compute LLR with numerical stability
                with np.errstate(divide="ignore", invalid="ignore"):
                    llr = np.log(llr_num / llr_den)
                    # Handle edge cases
                    llr = np.where(llr_den == 0, np.inf, llr)
                    llr = np.where(llr_num == 0, -np.inf, llr)
                    llr = np.where(np.isnan(llr), 0, llr)

                # Store LLR values
                output_bit_idx = self.num_bits_symbol - 1 - bit_pos
                demod_bits[output_bit_idx :: self.num_bits_symbol] = llr
                ...
                for bit_index in np.arange(self.num_bits_symbol):
                    ...
                    for bit_value, symbol in enumerate(self._constellation):
                        if (bit_value >> bit_index) & 1:
                            llr_num += exp(
                                (-(abs(current_symbol - symbol) ** 2)) / noise_var
                            )
                        else:
                            llr_den += exp(
                                (-(abs(current_symbol - symbol) ** 2)) / noise_var
                    ...
                    demod_bits[
                        i * self.num_bits_symbol + self.num_bits_symbol - 1 - bit_index
                    ] = log(llr_num / llr_den)
        else:
            raise ValueError('demod_type must be "hard" or "soft"')
    ...
    def plot_constellation(self):
        ...
        for symb in self.constellation:
            plt.text(symb.real + 0.2, symb.imag, self.demodulate(symb, "hard"))
...
class PSKModem(Modem):
    ...
    def __init__(self, m):
        ...
        super().__init__(exp(1j * np.arange(0, 2 * pi, 2 * pi / m)))
...
class QAMModem(Modem):
    ...
    def __init__(self, m):
        ...
        constellation = tile(
            hstack((pam, pam[::-1])), int(num_symb_pam) // 2
        ) * 1j + pam.repeat(num_symb_pam)
...
def ofdm_rx(y, nfft, nsc, cp_length):
    ...
    for i in range(0, num_ofdm_symbols):
        ...
        x_hat[:, i] = np.concatenate(
            (symbols_freq[-nsc / 2 :], symbols_freq[1 : (nsc / 2) + 1])
        )
...
def mimo_ml(y, h, constellation):
    ...
    _, n = h.shape
    m = len(constellation)
    x_ideal = np.empty((n, pow(m, n)), complex)
    for i in range(0, n):
...
def kbest(y, h, constellation, K, noise_var=0, output_type="hard", demode=None):
    ...
    if isinstance(constellation[0], complex):
        ...
        const_type = float
    X = np.empty((nb_rx, K * m), dtype=const_type)  # Set of current candidates
    d = np.tile(yt[:, None], (1, K * m))  # Corresponding distance vector
    d_tot = np.zeros(K * m, dtype=float)  # Corresponding total distance
    hyp = np.empty(K * m, dtype=const_type)  # Hypothesis vector
    ...
    for coor in range(nb_rx - 1, -1, -1):
        ...
        X[:, :nb_hyp] = np.tile(X[:, :nb_can], (1, m))
        d[:, :nb_hyp] = np.tile(d[:, :nb_can], (1, m))
        d_tot[:nb_hyp] = np.tile(d_tot[:nb_can], (1, m))

        # Make hypothesis
        hyp[:nb_hyp] = np.repeat(constellation, nb_can)
...
def best_first_detector(y, h, constellation, stack_size, noise_var, demode, llr_max):
    ...
    class _Node:
        ...
        def __init__(self, symb_vectors, partial_metrics):
            ...
            if len(partial_metrics) == 1:
                ...
                self.symb_vector = symb_vectors.reshape(
                    -1
                )  # Insure that self.symb_vector is a 1d-ndarray
                self.partial_metric = partial_metrics[0]
                self.best_sibling = None
            else:
                # Recursive call to build several nodes
                self.symb_vector = symb_vectors[:, 0].reshape(
                    -1
                )  # Insure that self.symb_vector is a 1d-ndarray
                self.partial_metric = partial_metrics[0]
        ...
        def expand(self, yt, r, constellation):
            ...
            children_symb_vectors = np.empty(
                (child_size, constellation.size), constellation.dtype
            )
            ...
            children_metric = (
                abs(
                    yt[-child_size]
                    - r[-child_size, -child_size:].dot(children_symb_vectors)
                )
                ** 2
            )
    ...
    stacks[-1].append(
        _Node(np.empty(0, constellation.dtype), np.array(0, float, ndmin=1)).expand(
            yt, r, constellation
        )
    )
    ...
    while any(stacks[1:]):
        ...
        for idx_next_stack in range(len(stacks) - 1):
            try:
                ...
                if map_bit_vector is None:
                    ...
                    bit_vector = demode(best_node.symb_vector).reshape(
                        -1, modulation_order
                    )
                    ...
                    try:
                        a2 = counter_hyp_metric[idx_this_stack:][
                ...
                if (
                    best_node.best_sibling is not None
                    and best_node.best_sibling.partial_metric <= radius
                ):
        ...
        if stacks[0]:
            if stacks[0][0].partial_metric < map_metric:
                ...
                map_bit_vector = demode(stacks[0][0].symb_vector).reshape(
                    -1, modulation_order
                )
                ...
                minimum(
                    counter_hyp_metric,
                    stacks[0][0].partial_metric,
                    out=counter_hyp_metric,
                )
            clip(
                counter_hyp_metric,
                map_metric - llr_max,
...
def max_log_approx(y, h, noise_var, pts_list, demode):
    ...
    bits = demode(pts_list.reshape(-1, order="F")).reshape(
        nb_pts, -1
    )  # Set of binary words (one word by row)

    # Prepare LLR computation
    nb_bits = bits.shape[1]
    LLR = np.empty(nb_bits)
