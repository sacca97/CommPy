...
def rrcosfilter_v2(N, alpha, Ts, Fs):
    """
    Generates a root raised cosine (RRC) filter (FIR) impulse response.
    ...
    for x in sample_num:
        t = (x-N/2)*T_delta
        if t == 0.0:
            h_rrc[x] = (1.0/np.sqrt(Ts)) * (1.0 - alpha + (4*alpha/np.pi))
        elif alpha != 0 and abs(t) == Ts/(4*alpha):
            h_rrc[x] = (alpha/np.sqrt(2*Ts)) * (((1+2/np.pi) * \
                    (np.sin(np.pi/(4*alpha)))) + ((1-2/np.pi)*(np.cos(np.pi/(4*alpha)))))
        else:
            numerator = np.sin(np.pi*t*(1-alpha)/Ts) + \
                       4*alpha*(t/Ts)*np.cos(np.pi*t*(1+alpha)/Ts)
            denominator = np.pi*t*(1-(4*alpha*t/Ts)**2)/Ts
            h_rrc[x] = (1.0/np.sqrt(Ts)) * (numerator/denominator)
